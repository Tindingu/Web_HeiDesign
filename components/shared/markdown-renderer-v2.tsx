...
function FaqAccordion({ faqs }: { faqs: MarkdownFaqItem[] }) {
        ...
        {faqs.map((item, index) => {
          ...
          return (
            <div key={`${item.question}-${index}`} className="py-5">
              ...
              >
                <span
                  className={`flex h-7 w-7 shrink-0 items-center justify-center text-[#9ABDC0] transition-transform duration-300 ${
                    isOpen ? "rotate-180" : ""
                  }`}
                  aria-hidden="true"
                >
...
export function MarkdownRendererV2({
  ...
  for (let index = 0; index < lines.length; index += 1) {
    if (isMainContentMarker(lines[index])) {
      flushBuffer();
      nodes.push(
        <div key={`main-marker-${nodes.length}`} className="not-prose my-12">
          <CompletedProjects
            projects={projects}
